<template>
  <view>
    <view
      class="tagBall"
      @touchstart.stop.prevent="touchstartscene"
      @touchend.stop.prevent="touchendscene"
...
export default {
  ...
  mounted() {
    ...
    // var tagEle = uni.createSelectorQuery(".tag");
    var tagEle;
    let array = [];
    uni
      .createSelectorQuery()
      ...
      .exec((result) => {
        ...
        for (var i = 0; i < tagEle.length; i++) {
          array.push({
            offsetWidth: tagEle[i].width,
            offsetHeight: tagEle[i].height,
          });
        }
      });
